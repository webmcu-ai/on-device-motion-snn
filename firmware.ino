...
// Shared by the training screens: split off VALIDATION_SAMPLES per class for
// testing, but never so many that a class is left with zero training
// examples - a class with N samples never holds out more than N-1, so a
// class you've only collected once still gets trained on.
int mySplitValidation(std::vector<TrainingItem>& valData){
  int valCount=0;
  myShuffle(myTrainingData);
  if(VALIDATION_SAMPLES>0){
    int totalPerClass[NUM_CLASSES]={};
    for(auto& it: myTrainingData) totalPerClass[it.label]++;
    int held[NUM_CLASSES]={}; std::vector<TrainingItem> trainOnly;
    for(auto& it: myTrainingData){
      int cap = max(totalPerClass[it.label]-1, 0);
      int want = min(VALIDATION_SAMPLES, cap);
      if(held[it.label]<want){valData.push_back(it);held[it.label]++;valCount++;} else trainOnly.push_back(it);
    }
