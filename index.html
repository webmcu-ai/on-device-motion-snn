...
//
// WHAT'S NEW IN v022
//   HOTKEY REDESIGN. Classes now pick their own letter instead of a menu
//   position digit, and that letter is matched case-insensitively (w or W
//   both collect the "W" class). "Still" and "Delete" have no natural
//   letter, so they keep the symbols '!' and '@'. This frees the alphabet
//   for classes, so the other commands moved off letters:
//     - The 7 actions (Train/Infer ANN/SNN, Train+Infer, AutoTrain ANN/SNN)
//       are now digits 1-7 instead of G/H/J/K/M/a/A.
//     - The 8 freeze toggles are now punctuation: % ^ & * for SNN
//       conv1/dense1/dense2/output, ( ) - = for the ANN equivalents.
//     - Verbose toggle ('v') is now '#'; reprint-menu ('m') is now '$'.
//     - Top-level "next item"/"select item" moved from t/l to , and . ,
//       since 'T' is now a class letter (t/l keep their old meaning of
//       "capture"/"exit" *inside* a class or training screen - those are
//       a different, unambiguous context and were left alone).
//   Adding a class is now just one line in myClasses[] (key, typed label,
//   SD folder). myValidateClassKeys() runs once at boot and warns over
//   Serial if two classes end up sharing a hotkey.
//   SD folder names are UNCHANGED from earlier versions (still "0Still",
//   "2W", etc.) even though the hotkeys and on-screen labels changed, so
//   samples collected on earlier firmware still load with no renaming.
//   Nothing about the ANN/SNN models, training, or the output engine
//   changed in this version.
//
// WHAT'S NEW IN v020
//   SNN double letters fixed ("RR", "OOO"...). Cause found: v017's "re-arm" rule
//   needed to SEE the letter's evidence fade before the same letter could be
//   typed again, but after every commit the network is reset and the output
//   engine is deaf for COOLDOWN_TICKS, so the fade happened during the deaf
//   period and was never observed. A second gesture made right away kept the
//   window full of that letter, the re-arm never fired, and the second letter
//   was silently dropped. The re-arm rule (REPEAT_REARM_PROB) is removed.
//   Why it is safe: a commit already resets the network, clears the vote window
//   and ignores COOLDOWN_TICKS, then a brand-new window of WINDOW_TICKS must
//   fill with fresh evidence. That is what stops the tail of one gesture from
//   typing twice. A NEW commit of the same letter needs a whole new window of
//   confident evidence, so one gesture = one letter; each further gesture types
//   again; a gesture that is HELD/repeated continuously types about once per
//   (COOLDOWN_TICKS + WINDOW_TICKS) ticks.
//   Also new: verbose mode prints a "[SNN-live] COMMIT ..." line with the
//   probability and margin each time a letter is typed, to help tuning.
//   The ANN is unchanged from v019.
//
// WHAT'S NEW IN v019
...
//   (In v019 the SNN behaviour was unchanged from v017; v020 replaces v017's
//   re-arm rule, see above.)
//
// WHAT'S NEW IN v016
...
//             modes. SNN: each commit resets the network and needs a fresh window of
//             confident evidence (after COOLDOWN_TICKS), so one gesture = one letter.
...
void myResetOutputDebounceSnn(){
  ...
  gRingPos=0; gRingFill=0; gCooldown=0; gLeadClass=0; gLeadProb=0;
}
...
// type extra letters.
// mayRepeat: true lets a letter follow itself (both SNN and ANN pass true).
// Still never repeats; Delete repeats after DELETE_REPEAT_MS.
void myCommitClass(int classIdx, bool mayRepeat){
...
void myUpdateOutputStringSnn(float* scores){
  ...
  if (gVerboseInfer) Serial.printf("[SNN-live] COMMIT %s p=%.2f margin=%.2f\n", myClasses[best].label, mean[best], mean[best]-mean[second]);
  myCommitClass(best, true);   // true: the same letter may follow itself (fresh window already required)
}
    ...
    if (tickCount % 8 == 0) {   // ~every 200 ms
      ...
      if (gVerboseInfer) {
        Serial.printf("[SNN-live] tick=%lu pred=%s  lead=%s p=%.2f%s  spikes/tick: c1=%.2f d1=%.2f d2=%.2f\n",
                      tickCount, myClasses[pred].label, myClasses[gLeadClass].label, gLeadProb,
                      gCooldown>0?" (cooldown)":"", c1Rate, d1Rate, d2Rate);
        u8g2.firstPage();
        do{ u8g2.setFont(u8g2_font_5x7_tf); u8g2.drawStr(0,8,"SNN live:"); u8g2.drawStr(0,18,myClasses[pred].label);
...
void setup(){
  ...
  delay(1000);
  Serial.println("\n=== XIAO ESP32-S3 Motion ML - Streaming SNN air-writing (imu-snn-v022) ===");
