...
//   - NEW: hitting Enter with nothing typed in Collect and Train+Infer
//     SNN now repeats your last real command - see SerialRepeatState -
//     so you no longer have to retype/resend 't' before every capture.
//     A \r\n pair is collapsed into one event so a normal send doesn't
//     double-fire.
//   - NEW 'a' auto-collect command (Collect screen only): captures
//     AUTO_COLLECT_COUNT (default 10) samples back-to-back, each with
//     the same 1s "get ready" warning as a manual capture and a
//     "Saved #N (auto k/10)" line after each one, so you get a beat and
//     some feedback between reps without tapping/typing every time. 3x
//     tap or 'l' aborts the burst early.
//
...
const int WINDOW_TICKS = 40;   // default 7;           // rolling window length, in ticks (~25ms/tick) - was LETTER_CONFIRM_TICKS
const float SNN_VOTE_MARGIN = 20.0f;  // default 4.0f;    // leader must beat the runner-up by this many points to commit (scale: a tick's full 1st-place vote = 3)
...
float LIF_THRESHOLD    = 0.75f;   // default 1.00f;   // spike threshold
float SNN_WEIGHT_SCALE = 1.5f;    //default 1.00f;   // scales synaptic current
float SNN_TRACE_LEAK   = 0.95f;   // decay of the readout trace used for classification
#define SURROGATE_K      10.0f         //default 25.0f         // fast-sigmoid surrogate steepness
...
// ======================================================
// SERIAL "REPEAT LAST COMMAND ON BLANK ENTER" HELPER
// Some of the blocking action loops below (Collect, Train+Infer SNN) use
// a single Serial byte as a one-shot trigger (commonly 't'). Hitting
// Enter with nothing typed in the Serial Monitor's input field sends
// just the line terminator - '\n', '\r', or '\r\n' depending on your
// Serial Monitor's line-ending setting ("No line ending" sends nothing
// at all and can't be detected this way) - with no real byte before it.
// resolve() turns that into a replay of whatever real byte was last
// sent, so you can keep hitting Enter to repeat an action instead of
// retyping/resending the same character every time.
//
// A \r\n (or \n\r) pair is collapsed into ONE logical line-end event -
// without that, a perfectly normal "t<CR><LF>" send would trigger 't'
// and then ALSO fire a spurious repeat when the second terminator byte
// arrived on the next loop pass.
// ======================================================
struct SerialRepeatState {
  char lastCmd = 0;
  bool lineHadCommand = false;
  bool prevWasLineEnd = false;
  // Feed it one raw byte from Serial.read(). Returns 0 if there's nothing
  // to act on this call, otherwise the effective command character -
  // either rawByte itself, or the replayed last command on a blank Enter.
...
// Does the "warn -> 1s delay -> capture -> report" sequence used by both a
// single manual capture and each step of an auto-collect burst below.
// progressSuffix is appended to the "Saved" line (e.g. " (auto 3/10)") -
// pass "" for a plain manual capture. Returns true on success (cc already
// incremented and the OLED already redrawn), false on an SD write failure.
...
// ======================================================
// AUTO-COLLECT: captures AUTO_COLLECT_COUNT samples back-to-back for
// classIdx, each with the same 1s "get ready" warning as a manual
// capture, so you get a beat to reset between reps without having to
// tap or type between every single one. Checks for an abort (3x tap or
// 'l'/'L') before each rep, so one mistake doesn't lock you into all 10.
// Triggered by 'a'/'A' from within Collect (see myActionCollect).
// ======================================================
#define AUTO_COLLECT_COUNT 10   // samples per auto-collect burst - tune to taste
void myActionAutoCollect(int classIdx, int &cc){
  Serial.printf("[Collect] Auto-collect: %d samples for '%s'. Starting auto in 1s...\n", AUTO_COLLECT_COUNT, myClassLabels[classIdx].c_str());
  delay(1000);
  int done=0;
  for(int i=0;i<AUTO_COLLECT_COUNT;i++){
    ...
    if(Serial.available()){ char c=Serial.read(); if(c=='l'||c=='L'){ Serial.printf("[Collect] Auto-collect stopped early ('l') after %d/%d.\n", done, AUTO_COLLECT_COUNT); return; } }
...
void myActionCollect(int classIdx){
  ...
  Serial.println("1 TAP (or send 't') = capture one 1s repetition. Send 'a' = auto-collect 10 in a row.");
  Serial.println("Hit Enter with nothing typed to repeat your last command. 3x TAP (or send 'l') = back to menu.");
  ...
  while(true){
    ...
    if(Serial.available()){
      ...
      if (c) {
        ...
        if(c=='a'||c=='A') myActionAutoCollect(classIdx, cc);
      }
    }
...
void myActionTrainInferSnn(){
  ...
  Serial.println("[SNN Train+Infer] Hit Enter with nothing typed to repeat your last command.");
  myResetTouchState();
  SerialRepeatState serialRepeat;
  while(true){
    int ta = myCheckTouchInput();
