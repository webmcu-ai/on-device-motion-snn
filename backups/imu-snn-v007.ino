...
// FULL MOTION / IMU ML — STREAMING SNN + SURROGATE GRADIENTS — v005
//
// Changes from v004 (functionality otherwise identical):
...
// ======================================================
// LIVE OUTPUT STRING — turns confirmed class predictions into a
// running line of text, for continuous "Infer SNN" streaming inference
// (where a raw prediction is available every single tick, and would
// otherwise fire on every tick a pose is held).
//
...
// slots later.
//
// A raw prediction only "counts" once it has been the same class for
// LETTER_CONFIRM_TICKS ticks in a row (debounces noise), and then it
// commits ONCE per stable stretch - i.e. it does nothing more until the
// prediction changes away to something else and re-stabilizes. That one
// rule is what stops a held gesture (or held stillness) from being
// committed hundreds of times while you hold the pose.
// ======================================================
String myOutputString = "";
int gLastCommittedClass = -1;        // last class that actually produced an action
int gPendingClass       = -1;        // raw class currently being confirmed
int gPendingStableTicks = 0;         // consecutive ticks gPendingClass has held
const int LETTER_CONFIRM_TICKS = 6;  // ~150ms at 25ms/tick - raise if letters fire too eagerly, lower if they feel laggy

String myClassText(int idx){ return myClassLabels[idx].substring(1); }

// Call when (re)entering a mode that should start confirming fresh -
// clears the debounce state but deliberately leaves myOutputString and
// gLastCommittedClass alone, so the transcript survives across
// entering/exiting Infer SNN.
void myResetOutputDebounce(){ gPendingClass=-1; gPendingStableTicks=0; }

// Call once per tick with the RAW (unfiltered) per-tick prediction from
// mySnnTick(). Internally debounced/edge-triggered as described above.
void myUpdateOutputString(int rawPred){
  if (rawPred == gPendingClass) gPendingStableTicks++;
  else { gPendingClass = rawPred; gPendingStableTicks = 1; }

  if (gPendingStableTicks < LETTER_CONFIRM_TICKS) return;   // not stable yet
  if (gPendingClass == gLastCommittedClass) return;         // already actioned this stretch

  gLastCommittedClass = gPendingClass;
  String text = myClassText(gPendingClass);

  if (text == "Still") {
    // Only add a space if the string isn't empty and doesn't already end in one.
...
// Also feeds every raw prediction into myUpdateOutputString() to build
...
void setup(){
  ...
  delay(1000);
  Serial.println("\n=== XIAO ESP32-S3 Motion ML — Streaming SNN + Surrogate Gradients v005 ===");
