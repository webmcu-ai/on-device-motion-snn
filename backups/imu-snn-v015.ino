...
//   a          auto-capture 10 gestures in a row
//   Enter      (blank) repeats the last command
//   l          exit / select        3x tap also exits
...
//   A S D F    freeze SNN conv1 / dense1 / dense2 / output
//   a s d f    freeze ANN conv1 / dense1 / dense2 / output
//
...
const int   ANN_CONFIRM_WINDOWS = 2;     // ANN: same answer this many 1-second windows in a row
...
bool mySnnLoadWeights(){
  if(!mySDavailable||!SD.exists(SNN_WEIGHT_FILE)) return false;
...
void mySnnSaveWeights(){
  if(!mySDavailable) return; if(!SD.exists("/header")) SD.mkdir("/header");
  File f=SD.open(SNN_WEIGHT_FILE,FILE_WRITE);
  if(f){
...
//   other  -> append the class letter; not repeated until something else
//             (e.g. Still) has been committed in between
...
void myCommitClass(int classIdx){
  ...
  if (classIdx == gLastCommittedClass) {
    if (!(isDelete && millis()-gLastCommitMs >= DELETE_REPEAT_MS)) return;
  }
...
// ---- ANN path: the same answer for ANN_CONFIRM_WINDOWS windows in a row ----
...
// 3x tap or 'l' stops the burst early.
void myActionAutoCollect(int classIdx, int &cc){
  Serial.printf("[Collect] Auto-collect: %d samples for '%s'. Starting in 1s...\n", AUTO_COLLECT_COUNT, myClassLabels[classIdx].c_str());
  delay(1000);
  int done=0;
...
void myActionCollect(int classIdx){
  ...
  Serial.println("1 TAP (or 't') = capture one 1s gesture. 'a' = auto-collect 10 in a row.");
  Serial.println("Blank Enter repeats your last command. 3x TAP (or 'l') = back to menu.");
...
void myActionTrainAnn(){
  ...
  Serial.printf("[ANN] Training on %d gestures, holding out %d for validation. %d epochs, lr=%.4f, batch=%d.\n",
  ...
  for(int epoch=0;epoch<TARGET_EPOCHS;epoch++){
    ...
    Serial.printf("[ANN] Epoch %d/%d Loss=%.4f TrainAcc=%.1f%% ValAcc=%.1f%%%s\n",epoch+1,TARGET_EPOCHS,loss/nTrain,trainAcc,valAcc, isBest?"  <- best so far":"");
    u8g2.firstPage();
  ...
  Serial.printf("[ANN] Done in %.1fs. Best %s=%.1f%% at epoch %d. Weights saved.\n",
                (millis()-trainStartMs)/1000.0f, valCount>0?"ValAcc":"TrainAcc", bestAcc, bestEpoch);
...
// Windowed inference: one prediction per 1-second window, majority vote over the last 3.
...
// ======================================================================
// Train SNN: BPTT on the saved CSV gestures. Gradients are averaged over
...
void myActionTrainSnn(){
  ...
  std::vector<TrainingItem> valData;
  int valCount = mySplitValidation(valData);
  Serial.printf("[SNN] Training on %d gestures, holding out %d for validation. %d epochs, lr=%.4f, batch=%d.\n",
                (int)myTrainingData.size(), valCount, TARGET_EPOCHS, LEARNING_RATE, BATCH_SIZE);
  Serial.printf("[SNN] Frozen layers: conv1=%d dense1=%d dense2=%d output=%d | LIF_THRESHOLD=%.2f LIF_LEAK=%.2f SPIKE_DELTA_G=%.3f\n",
                gSnn.freezeConv1, gSnn.freezeDense1, gSnn.freezeDense2, gSnn.freezeOutput, LIF_THRESHOLD, LIF_LEAK, SPIKE_DELTA_G);
  unsigned long trainStartMs = millis();
  float bestAcc=-1; int bestEpoch=-1;
  const int nTrain = max((int)myTrainingData.size(),1);
  for(int epoch=0; epoch<TARGET_EPOCHS; epoch++){
    ...
    Serial.printf("[SNN] Epoch %d/%d Loss=%.4f TrainAcc=%.1f%% ValAcc=%.1f%%%s (last-gesture spikes: c1=%ld d1=%ld d2=%ld)%s\n",
      epoch+1,TARGET_EPOCHS, lossSum/nTrain, trainAcc, valAcc, isBest?"  <- best so far":"",
...
// Digits 0-9 always select that class; letters run the five actions.
...
void myPrintStatus(){
  ...
  Serial.printf("Training: lr=%.4f batch=%d epochs=%d valSamples=%d\n", LEARNING_RATE, BATCH_SIZE, TARGET_EPOCHS, VALIDATION_SAMPLES);
  ...
  Serial.printf("Output engine: SNN window=%d ticks, min prob=%.2f, margin=%.2f, cooldown=%d ticks | ANN confirm=%d windows | Delete repeat=%lums\n",
                WINDOW_TICKS, COMMIT_MIN_PROB, COMMIT_MARGIN, COOLDOWN_TICKS, ANN_CONFIRM_WINDOWS, DELETE_REPEAT_MS);
...
void myDrawMenu(){
  ...
  Serial.println("Hotkeys: 0-9=class, G/H/J/K/M=Train ANN/Infer ANN/Train SNN/Infer SNN/Train+Infer SNN");
  Serial.println("Other: A/S/D/F=SNN freeze  a/s/d/f=ANN freeze  ?=status  v=verbose toggle  m=reprint menu  t=next  l=select");
  u8g2.firstPage();
  do{
    u8g2.setFont(u8g2_font_5x7_tf);
...
void setup(){
  ...
  delay(1000);
  Serial.println("\n=== XIAO ESP32-S3 Motion ML - Streaming SNN air-writing (imu-snn-v015) ===");
  Serial.println("Hotkeys: 0-9 = class, G/H/J/K/M = Train ANN/Infer ANN/Train SNN/Infer SNN/Train+Infer SNN, m = menu, ? = status, v = verbose toggle.");
  ...
  myAnnLoadWeights();
  if(!mySnnLoadWeights()) Serial.println("No v015 SNN weights found - run Train SNN (older SNN weights are not compatible).");
  myRefreshActiveClassMask();
