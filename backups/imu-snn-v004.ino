...
//
// ---- Original v003 header follows, unchanged ----
//
...
void myActionCollect(int classIdx){
  ...
  Serial.println("TAP (or send 't') = capture one 1s repetition. HOLD (or send 'l') = back to menu.");
  ...
  while(true){
    if(Serial.available()){
      ...
      if(c=='t'||c=='T'){
        ...
        delay(1000);
        if(myCaptureSample(classIdx)){ cc++; Serial.printf("[Collect] Sample #%d saved for '%s'.\n", cc, myClassLabels[classIdx].c_str()); }
        else Serial.println("[Collect] Capture FAILED (SD write error).");
      }
    }
...
void myActionInferAnn(){
  ...
  Serial.println("Move the sensor. HOLD or send 'l' to return to menu.");
